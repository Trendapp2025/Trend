...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/top-predictors/:monthYear", async (req, res) => {
    try {
      ...
      const topPredictors = await storage.getMonthlyTopPredictors(
        monthYear,
        limit
      );
  ...
  app.get("/api/admin/users", async (req, res) => {
    ...
    if (req.user!.username !== "admin") {
      console.log(
        `Utente ${req.user!.username} (id: ${
          req.user!.id
        }) ha tentato di accedere all'endpoint admin senza privilegi`
      );
      return res.status(403).json({ message: "Not authorized" });
    }
    ...
    try {
      console.log(
        `Admin ${req.user!.username} (id: ${
          req.user!.id
        }) accede all'elenco utenti`
      );
