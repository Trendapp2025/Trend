...
export default function OpinionForm({ assetId, assetSymbol, userId }: OpinionFormProps) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: OpinionFormData) => {
      ...
      const res = await apiRequest("POST", `/api/assets/${assetSymbol}/opinions`, payload);
      return res.json();
    },
    onSuccess: () => {
      toast({
      ...
      form.reset();
      queryClient.invalidateQueries({ queryKey: [`/api/assets/${assetSymbol}/opinions`] });
      queryClient.invalidateQueries({ queryKey: [`/api/assets/${assetSymbol}`] });
    },
    onError: (error: Error) => {
      toast({
